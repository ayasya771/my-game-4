...
if(gameState==="play"){
 ...
 timer=timer-0.02;
  ...
  shrub1.y= random(800,1200);
}
if(frameCount % 3000 === 0){
  
  shrub2.y = random(200,800);
  shrub2.x = random(1000, 1400);
}
...
if(ambulance.isTouching(shrub2)){
  track.velocityY = track.velocityY + 3;
}

if (track.velocityY === 30){
 gameState = "end";
}
...
if(track.velocityY === 0){
  textSize(40);
  fill("black");
  text("You Were Not Able to save the patient in due time",800,800);
}
  fill("black");
  text("TIMER:"+Math.round(timer),windowWidth-300,50);
  track.visible = true;
...
if (gameState === "end"){
  ...
  timer = timer - 0;
  player.addImage(patientImg2);
  
  if(player.x === player.x + 100){
    ...
    textSize(40);
    fill("Black");
    text("You Saved The Patient's Life in ",)
  }
